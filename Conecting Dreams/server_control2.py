...
# Definir la red
nsfnet = nx.Graph()
nsfnet.add_node(('127.0.0.1', 12001))  # V  Puerto 12001
nsfnet.add_node(('127.0.0.1', 12002))  # W  Puerto 12002
nsfnet.add_node(('127.0.0.1', 12003))  # X Puerto 12003
nsfnet.add_node(('127.0.0.1', 12004))  # Y Puerto 12004
nsfnet.add_edge(('127.0.0.1', 12001), ('127.0.0.1', 12002), weight=0.33)
nsfnet.add_edge(('127.0.0.1', 12001), ('127.0.0.1', 12003), weight=0.5)
nsfnet.add_edge(('127.0.0.1', 12002), ('127.0.0.1', 12003), weight=0.33)
nsfnet.add_edge(('127.0.0.1', 12002), ('127.0.0.1', 12004), weight=1)
nsfnet.add_edge(('127.0.0.1', 12003), ('127.0.0.1', 12004), weight=1)

def save_shortest_paths(network):
    shortest_paths = {}
    for node in nsfnet.nodes():
        ip, port = node[0], node[1]
        shortest_paths[f"{ip}_{port}"] = {}  # Convertir la clave de tupla a string
        for target_node in nsfnet.nodes():
            if node != target_node:
                shortest_path = nx.shortest_path(network, source=node, target=target_node)
                shortest_paths[f"{ip}_{port}"][f"{target_node[0]}_{target_node[1]}"] = shortest_path
